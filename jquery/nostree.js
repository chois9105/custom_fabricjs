...
function NTreeNode( treeView, parentNode , text, url, target){
	...
	this.getContent = function(){
		...
		function buttonSquare(node){
			...
			return '<td valign=\"middle\" width="'+w+'"><a href="javascript:none()" onclick="javascript:NTrees[\''+node.treeView.name+'\'].expandNode('+node.index+')"><img name=\''+node.id()+'nb\' id=\''+node.id()+'nb\' src="../../../../../MediaHub/CMS/cms/content/news/' + img + '" width="'+w+'" height="'+h+'" border=0></a></td>\n';
		}
		...
		if ( this.treeView.fmt.showF)
			s += itemSquare(this);
		s += '<td nowrap=\"1\"><a class="'+this.treeView.fmt.nstyle(ll)+'" href="../../../../../MediaHub/CMS/cms/content/news/'+this.url+'" target="'+this.target+'" onclick="javascript:NTrees[\''+this.treeView.name+'\'].expandNode('+this.index+')">'+this.text+'</a></td></tr></table>';
		return s;
	}
